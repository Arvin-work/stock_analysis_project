...
def sliding_window_fft(data, window_size=30, overlap=0.5):
    ...
    # 使用扩展数据边界
    pad_siz = window_size // 2
    padded_data = np.pad(data, (pad_siz, pad_siz), mode='edge')
    ...
    for i in range(0, n, step_size):
        ...
        padded_end = min(start_idx + window_size, len(padded_data))
        window_data = padded_data[padded_start:padded_end]

        # 如果数据不足，从末尾补足
        if len(window_data) < window_size:
            needed = window_size - len(window_data)
            window_data = np.concatenate([window_data, data[-needed:]])

        fft_windows = np.fft.fft(window_data - np.mean(window_data))
        n_keep = len(window_data) // 2
        fft_windows[n_keep:] = 0  # 保留低频成分

        reconstructed_window = np.fft.ifft(fft_windows).real + np.mean(window_data)
        hanning_window = np.hanning(len(reconstructed_window))
        actual_end = min(end_idx, n)
        slice_length = actual_end - start_idx
        reconstructed[start_idx:actual_end] += reconstructed_window[:slice_length] * hanning_window[:slice_length]
        weights[start_idx:actual_end] += hanning_window[:slice_length]
...
if train_fit_basic is not None:
    ...
    # 计算拟合优度
    ss_res_basic = np.sum(residuals_basic**2)
    ss_tot_basic = np.sum((y_train - np.mean(y_train))**2)
    r2_basic = 1 - (ss_res_basic / ss_tot_basic)
    # 调整R²
    n_params_basic = len(initial_guess_basic)
    n_samples = len(y_train)
    adj_r2_basic = 1 - (1 - r2_basic) * (n_samples - 1) / (n_samples - n_params_basic - 1)
    print(f"基本FFT - 训练集拟合优度 R²: {r2_basic:.44}, 调整后 R²: {adj_r2_basic:.4f}")

    residual_methods.append(("基本FFT", residuals_basic, r2_basic, adj_r2_basic))

if train_fit_simple is not None:
    residuals_simple = y_train - train_fit_simple
    ss_res_simple = np.sum(residuals_simple**2)
    ss_tot_simple = np.sum((y_train - np.mean(y_train))**2)
    r2_simple = 1 - (ss_res_simple / ss_tot_simple)
    n_params_simple = len(initial_guess_simple)
    adj_r2_simple = 1 - (1 - r2_basic) * (n_samples - 1) / (n_samples - n_params_simple - 1)
    print(f"简化FFT - 训练集拟合优度 R²: {r2_basic:.4f}, 调整后 R²: {adj_r2_basic:.4f}")

    residual_methods.append(("简化FFT", residuals_simple, r2_simple, adj_r2_simple))

if train_fit_sliding is not None:
    residuals_sliding = y_train - train_fit_sliding
    ss_res_sliding = np.sum(residuals_sliding**2)
    ss_tot_sliding = np.sum((y_train - np.mean(y_train))**2)
    r2_sliding = 1 - (ss_res_sliding / ss_tot_sliding)
    # 滑动窗口FFT没有明确的参数数量，使用窗口大小作为参考
    adj_r2_sliding = r2_sliding  # 简化处理
    residual_methods.append(("滑动窗口FFT", residuals_sliding, r2_sliding, adj_r2_sliding))

if train_fit_trend is not None:
    residuals_trend = y_train - train_fit_trend
    ss_res_trend = np.sum(residuals_trend**2)
    ss_tot_trend = np.sum((y_train - np.mean(y_train))**2)
    r2_trend = 1 - (ss_res_trend / ss_tot_trend)
    n_params_trend = len(initial_guess_trend)
    adj_r2_trend = 1 - (1 - r2_trend) * (n_samples - 1) / (n_samples - n_params_trend - 1)
    residual_methods.append(("带趋势FFT", residuals_trend, r2_trend, adj_r2_trend))

# 绘制残差图
for i, (method_name, residuals, r2, adj_r2) in enumerate(residual_methods):
    plt.subplot(2, 2, i+1)
